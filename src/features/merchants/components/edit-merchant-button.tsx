...
import { updateMerchant } from "@/app/d/merchants/action";
import type { Merchant } from "../types";

export function EditMerchantButton({ merchant }: { merchant: Merchant }) {
  ...
  return (
    <>
      ...
      {open && <EditMerchantModal merchant={merchant} onClose={() => setOpen(false)} />}
    </>
  );
}
...
function EditMerchantModal({ merchant, onClose }: { merchant: Merchant; onClose: () => void }) {
  ...
  const [wilaya, setWilaya] = useState(merchant.wilaya ?? "");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6 w-full max-w-md shadow-xl">
        <h2 className="text-lg font-bold text-[var(--text)] mb-5">{t("edit_merchant")}</h2>
