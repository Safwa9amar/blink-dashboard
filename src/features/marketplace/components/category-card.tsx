...
import { CATEGORY_STATUS, type TFn, type Category } from "../data";
...
export function CategoryCard({
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-4">
      ...
      <div className="mt-3 font-bold text-text">{category.name}</div>
      <div className="text-subtext text-xs mt-0.5">
        {category.slug} · {category.storeCount} {t("col.products")}
      </div>
      <div className="mt-3">
        <Badge variant={CATEGORY_STATUS[category.status]}>
          {t("category_status." + category.status)}
        </Badge>
      </div>
    </div>
  );
