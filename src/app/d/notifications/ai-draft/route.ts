...
import { notifDraftMessages } from "@/features/notifications/ai";

// Streaming notification-draft generator. Streams the model's tokens to the
// browser as newline-delimited JSON so the operator sees generation live —
...
// Mirrors src/app/d/news/ai-draft/route.ts.
...
export async function POST(req: Request) {
  ...
  if (!body.topic?.trim()) {
    return Response.json({ error: "Describe what the notification should say." }, { status: 400 });
  }

  const messages = notifDraftMessages({
    topic: body.topic,
    type: body.type,
